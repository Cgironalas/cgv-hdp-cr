...
public:
	abst_managed_list(const std::string& _element_name) : element_name(_element_name) {}
	void attach_list(abst_managed_list& attachment) { attachments.insert(&attachment); }
	void detach_list(abst_managed_list& attachment) { attachments.erase(&attachment); }
	virtual size_t size() const = 0;
	virtual const void* get_element_ptr(size_t index) const { return 0; }
	virtual std::string get_element_name(size_t index) const { return element_name + " " + cgv::utils::to_string(index); }
	virtual std::string* get_element_name_ptr(size_t index) { return 0; }
