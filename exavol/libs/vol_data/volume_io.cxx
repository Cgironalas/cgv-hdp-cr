...
bool write_vox_header(const std::string& file_name,  const volume& V)
{
	...
	os << "Size:    " << V.get_dimensions()(0) << "x" << V.get_dimensions()(1) << "x" << V.get_dimensions()(2);
	...
	switch (V.get_format().get_standard_component_format()) {
	...
	os << std::endl;
	volume::extent_type scaling = V.get_box().get_extent();
	os << "Scaling: " << scaling(0) << "x" << scaling(1) << "x" << scaling(2) << std::endl;
	return !os.fail();
}
...
bool write_vox(const std::string& file_name, const volume& V)
{
	if (!write_vox_header(cgv::utils::file::drop_extension(file_name) + ".hd", V))
		return false;
